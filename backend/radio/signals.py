...
def constance_updated(changes, request=None, **kwargs):
    if any(change.startswith("ICECAST_") and change != "ICECAST_URL" for change in changes):
        status = restart_container("radio-uplink", fail_silently=True)
        if request:
            if status:
                messages.info(request, "radio-uplink container restarted")
            else:
                messages.error(request, "There was an error restarting the radio-uplink container. See server logs.")
