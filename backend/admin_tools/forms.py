...
class SendTextMessageForm(forms.Form):
    ...
    message = forms.CharField(widget=forms.Textarea(attrs={'rows': 4, 'x-model': 'message', 'placeholder': 'Enter message body here...'}))
    audience = forms.ChoiceField(choices=AUDIENCE_CHOICES, initial='single', widget=forms.Select(attrs={'x-model': 'audience'}))
